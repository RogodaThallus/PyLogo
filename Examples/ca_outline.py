...
class CA_World(OnOffWorld):
    ...
    def generate_new_line_from_current_line(self, prev_line):
        ...
        gen_line = [''.join(map(str, prev_line[i:i + 3])) for i in range(len(prev_line) - 2)]
        new_line = [int(gui_get(cell)) for cell in gen_line]
        return new_line
    ...
    def set_display_from_lines(self):
        ...
        for (ca_line, patch_row) in ca_lines_patch_rows:
            # We display all the symbols starting at left_ca_line_index.
    ...
    def step(self):
        ...
        """
        new_line = self.generate_new_line_from_current_line(copy(self.ca_lines[-1]))
